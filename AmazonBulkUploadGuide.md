...
## 🆕 Flujo 2026 (validado 21/04) — Cambios vs flujo anterior

Amazon rediseñó el Bulk Operations en Q1 2026. 4 cambios clave respecto al flujo que validamos el 07/04:

### 1. Batch IDs ahora son UUIDs

| Antes (2025 y Q1 2026) | Ahora (Q2 2026+) |
|---|---|
| `158936020550` (numérico, 12 dígitos) | `cee6c2f5-520f-45d2-b769-c70f49e95776` (UUID) |

Guardar siempre el UUID completo para trazabilidad. No acortar.

### 2. Flujo asíncrono con 3 estados

El upload ya no es inmediato. El status pasa por:

```
Uploading → Processing → Success / Failed
```

Refrescar la página cada 30-60s (no siempre actualiza solo). Tiempo total: 30s a varios minutos según cantidad de filas.

### 3. Result file NO trae IDs rellenados

**CRÍTICO — diferencia grande vs flujo anterior.**

En el flujo antiguo, el result file descargable venía con los IDs numéricos de Amazon (`Campaign ID`, `Ad Group ID`, `Ad ID`, `Product Targeting ID`) rellenados como prueba de creación.

En 2026, el result file **devuelve el archivo igual a como se subió** — los campos ID siguen con los nombres que usamos, no con IDs numéricos.

**Consecuencia operativa:** `Status: Success` en la UI **no prueba por sí solo** que las entidades fueron creadas. La prueba definitiva es verificar visualmente en Campaign Manager que las campañas aparecen con sus targets y negatives.

### 4. Tab History separada

La historia de uploads ahora está en un tab dedicado **History** en el menú lateral izquierdo (arriba de Bulk Operations). Antes estaba integrada en la misma vista del upload.

---

## ✅ Verificación post-upload (flujo 2026)

Después de subir el bulk, NO alcanza con ver `Status: Success`. Siempre ejecutar estos 6 pasos:

1. **Campaigns** (menú izquierdo) → filtrar por nombre de la nueva campaña
2. **Verificar** que aparece: State = Enabled, Daily Budget correcto
3. **Click en la campaña** → tab Targeting → confirmar N targets creados
4. **Tab Negative targeting** → confirmar N negative keywords creadas
5. **Asignar Portfolio manualmente** (el bulk no puede asignar portfolio — limitación API desde siempre)
6. **Screenshot** de las campañas activas + anotar UUID de batch para auditoría

Si falta algún paso y al día 14 tratás de evaluar, te podés encontrar con campañas "fantasma" sin portfolio (no las podés filtrar bien) o sin negatives (canibalizan DEFEND).
...
| Status: Success pero campañas no aparecen | "Success" en flujo 2026 solo confirma que el archivo pasó validación — NO prueba creación | Verificar visualmente en Campaigns → filtrar por nombre exacto |
| Result file descargado sin IDs numéricos | Comportamiento esperado en flujo 2026 — no es error | Ignorar y verificar en Campaign Manager directamente |

---

**Validado:**
- 07 abril 2026 — Batch ID `158936020550` — 52/52 rows processed (M&B Women Non-Branded, flujo antiguo síncrono)
- 21 abril 2026 — Batch ID `cee6c2f5-520f-45d2-b769-c70f49e95776` — 38 rows (M&B PAT Women Premium CR+VN, **flujo 2026 async**)
