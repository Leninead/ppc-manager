...
**🔑 Aprendizaje operativo (consolidador):** el 12/08 se cerró la jornada reportando todo sincronizado, con este frente pendiente. El handoff estaba en `resumenes-2026-08-11.txt`, untracked y **no cubierto por `.gitignore`** — invisible salvo que alguien lo abriera. Es la segunda vez que aparece un `resumenes-*.txt` suelto en la raíz (existe `_archive/docs/resumenes-2026-05-26.txt`, que llegó a trackearse por accidente en `5554d2d`). **RESUELTO 13/08 (`80f1d64`): `resumenes-*.txt` en `.gitignore` + barrido `git worktree list` como paso del cierre** (con negación `!_archive/docs/resumenes-*.txt` para conservar los ya archivados; el handoff del 11/08 quedó movido a `_archive/docs/`).
