---
tipo: doc
actualizado: 2026-08-03
---

# Vault de Capybaras Agency — Índice de entrada

Este es el cerebro operativo de la agencia. Si sos dev nuevo, leé en este orden:

1. SETUP-DEV.md (raíz del repo) — cómo montar el entorno.
2. notes/CLAUDE.md — convenciones del vault.
3. notes/state/STATE-agencia.md — estado actual de todo.
4. notes/sops/dev/ — cómo trabajamos el código.

## Mapa de carpetas

Las carpetas más usadas: state/ (estado actual, el doc más vivo), brands/ (una por cliente), daily/ (sesiones), sops/ (procesos: dev/ · usuario/ · agencia/), modules/ (planes por módulo).

El mapa completo y actualizado de las 14 carpetas vive en notes/CLAUDE.md (sección "Estructura de carpetas"), que es la fuente canónica — para no mantener dos listas que divergen.
