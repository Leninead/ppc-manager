...
- El uploader dejó de estar en `render()`: los datos llegan de `render_source_picker()` (`modules/pages/search_term_source.py`), que devuelve un `SearchTermSource` (`core/search_term/frame.py`) o `None`. `df_raw = source.frame`; de ahí para abajo el módulo no sabe de dónde vino la tabla.
...
- **Chat con tools (fase 3)**: desde 2026-09-15 las herramientas de DataDive las usa el orquestador del chat de la app (su frontmatter declara `datadive`); el `tools:` del agente `datadive` se conserva. `runtime.ask_followup` manda `tools:["datadive"]` al provider, que expone 6 tools MCP read-only in-process (list_niches, get_niche_keywords, get_niche_competitors, niches_for_asins, list_rank_radars, get_quota; resultados truncados). `niches_for_asins` (2026-09-25) busca en todos los niches de la organización los que listan unos ASINs o una marca entre sus competidores; lee los competidores de cada niche una vez por fecha de research y los guarda en memoria. Solo el CHAT es agéntico — el análisis nunca lleva tools. Server-side vive en capybaras-ai-provider (`app/datadive_tools.py`, rama feat/datadive-mcp-tools) con `DATADIVE_API_KEY` en su .env.
...
- Sin UI: `ReportProvider(rest).profiles()` y `ReportProvider(rest).search_terms(option, desde, hasta)`
  ...
  (`core/search_term/frame.console_columns`) más los IDs ocultos al final.
