...
def render():
    ...
    with bid_tab3:
        ...
        bid_labels = _BID_LABELS.get(ai_lang, _BID_LABELS["es"])
        ...
        if not AI_ENABLED:
            ...
            from ai.agents.bid_optimizer import context as bid_ctx

            records = bid_ai_records(df_asin, cols, col_asin, bid_ctx.MAX_ASINS)
            if not records:
                st.info(bid_labels["no_rows"])
            else:
                labels = ai_tab.ai_labels(ai_lang, bid_labels)
                st.markdown(ai_tab.AI_CSS, unsafe_allow_html=True)
                payload = bid_ctx.BidData(
                    account_label=source.label,
                    period_label=_period_label(source),
                    currency_code=currency_code,
                    target_acos=int(target_acos),
                    price_source=precio_fuente,
                    asin_source=asin_source,
                    total_asins=len(df_asin),
                    asins=records,
                    campaigns=camp_rows[:bid_ctx.MAX_CAMPAIGNS],
                    idioma=ai_lang)
                # El target ACoS y el Inventory Report los pone el AM acá: el análisis espera el click.
                analysis = ai_tab.resolve_analysis(
                    slug="bid_optimizer", payload=payload, file_signature=source.signature,
                    labels=labels, show_previous=False, auto_fire=False)
                if analysis is not None:
                    render_records = ai_tab.records_for_render(
                        "bid_optimizer", analysis, payload, records)
                    ai_tab.render_analysis(
                        analysis, slug="bid_optimizer", labels=labels,
                        render_result=partial(_render_bid_ai_result, records=render_records,
                                              labels=labels, currency_code=currency_code))
                    ai_tab.publish_analysis_to_chat(
                        "bid_optimizer", analysis, payload, module_label="Bid Optimizer",
                        subject=source.label,
                        reading=lambda a, _rec=render_records: bid_chat_document.reading_text(
                            a.result, _rec),
                        annotate=partial(ai_tab.annotate_row_ids,
                                         labels_by_id=bid_row_labels(render_records)))
