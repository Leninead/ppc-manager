...
def _parse_merchanspring_pdf(file):
    ...
    # ── P&L ──────────────────────────────────────────────────────────────
    # Locate the P&L block by its anchor term; use a 3 000-char window around it
    pnl_anchor = re.search(
        r'(?:Shipped product sales|Net revenue|PROFIT\b)', full_text, re.IGNORECASE
    )
    if pnl_anchor:
        _ps = max(0, pnl_anchor.start() - 300)
        pnl_text = full_text[_ps: _ps + 4000]
    else:
        # Fall back: try the last third of the document
        pnl_text = full_text[max(0, len(full_text) - 4000):]

    def _pnl_val(text, label):
        m = re.search(rf'{re.escape(label)}\s+(-?[\$\d,\.]+|-)\s+', text)
    ...
    for display, key in pnl_items:
        ...
        if val != "-":
            pnl_found += 1
        pct_m = re.search(rf'{re.escape(key)}\s+(?:-?[\$\d,\.]+|-)\s+(-?[\d\.]+%|-)', pnl_text)
        pnl_rows.append({"Item": display, "Amount ($)": val, "% of Net Revenue": pct_m.group(1) if pct_m else "-"})

    result["pnl_df"] = pd.DataFrame(pnl_rows) if pnl_found > 0 else pd.DataFrame()

    profit_pct_m = re.search(r'Profit %\s+([\d\.]+%)',    pnl_text)
    orders_m     = re.search(r'Orders\s+(\d+)',            pnl_text)
    units_sold_m = re.search(r'\nUnits\s+(\d+)',           pnl_text)
    tacos_m      = re.search(r'TACOS %\s+([\d\.]+%|-)',    pnl_text)
    result["pnl_metrics"] = {
    ...
    slog.append(("✅" if pnl_found > 0 else "❌", "P&L",
                 f"{pnl_found} line items found" if pnl_found > 0 else "P&L section not found in PDF"))

    # ── Product-level profitability ───────────────────────────────────────
    prod_sec_m = re.search(
        r'PRODUCT\s+SHIPPED PRODUCT SALES.*?\n(.*?)(?=HEALTH STATUS|ACTIVE PRODUCTS|\Z)',
        full_text, re.DOTALL
    )
...
with st.sidebar:
    st.title("🚀 PPC Manager")
    st.caption("Amazon Advertising Analytics")
    st.divider()
    selected = st.radio("Módulo", _PAGES, label_visibility="collapsed")
...
if selected == "🔬 Reportes Atom 11":
    ...
    with col_u1:
        file_a1 = st.file_uploader("Periodo 1 \u2014 anterior (o unico archivo)", type=["xlsx", "csv"], key="atom11_f1")
    with col_u2:
        file_a2 = st.file_uploader("Periodo 2 \u2014 actual (opcional, para comparacion)", type=["xlsx", "csv"], key="atom11_f2")
