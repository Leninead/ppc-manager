...
### Added
- **Capa IA del STR (M2) via ai-provider (2026-08-28 → 2026-08-31)** — paquete `ai/` nuevo (config, client HTTP al gateway con log JSONL local, runtime con corrida en segundo plano + registro idempotente por digest compartido entre usuarios, agente en `ai/agents/str/` con prompt versionado — Opus 5, structured output forzado por schema — y contrato de datos `StrData`). El Tab "Analisis IA" pierde botón y sliders duplicados: el análisis dispara solo al terminar el pipeline, y se imprime con jerarquía (chips de advertencias, card de síntesis, tablas HTML con lectura IA que envuelve texto y apilan como tarjetas en mobile, diagnóstico por campaña con sangradores obligatorios). Cambios de datos/parámetros → aviso "Los datos cambiaron" + Recalcular, resultado viejo visible. Idioma del análisis atado al selector global nuevo del sidebar (`app_lang`, ES/EN). Chat flotante reusable (`core/ai_chat.py` + reglas compartidas `ai/agents/_shared/chat.md` que el runtime anexa a cualquier agente): burbuja fija, respuestas ≤100 palabras con cierre accionable, header con ícono copiar-hilo (iOS-safe, etiqueta con el nombre del usuario logueado), ramas de conversación aisladas por usuario vía fork de sesión del provider. Prompt endurecido con la review multi-agente sobre corridas reales: flag determinista de calidad de datos (columna de costo ausente → R3 suprimida declarada, sin especulación), CTR por fila en candidatos R5, régimen de evidencia R2 vs R5, mecánica correcta de negativeExact, compatibilidades a nivel cuenta, reglas duras de estilo (row_ids, sin códigos internos, formatos únicos) y marca visual "revisar categoría" ante incoherencias marca/categoría. El resto del módulo (tabs 1-3 y 5) queda byte a byte; `core/ai_analyze.py` sigue vivo para los otros módulos.
